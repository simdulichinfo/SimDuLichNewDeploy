...
export async function confirmPaymentAdmin(supabase, orderCode) {
  ...
  if (order.payment_status !== 'paid') {
    // Write the audit trail first: if the order were flipped to 'paid' first and this
    // insert failed, a retry would short-circuit on the 'paid' check and the
    // payment_transactions row would be lost with no way to recover it here.
