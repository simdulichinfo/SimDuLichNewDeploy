...
export async function runSmartImport(supabase, buffer, pricingOptions = {}, { commit = false } = {}) {
  ...
  for (let i = 0; i < parsedRows.length; i++) {
    ...
    // Guard closest to the actual DB write: whichever of the 4 pricing rules
    // produced this priceBuy, a non-finite or non-positive value must never
    // reach the products upsert (it would go live at price 0/negative and
    // poison min_price_by_country()'s MIN aggregate). Treat it the same way
    // as any other unprocessable row (see parseError above): mark it failed
    // in the preview and skip registering its category, since no product
    // will be written for it.
    if (!Number.isFinite(priceBuy) || priceBuy <= 0) {
      errorCount += 1;
      rowPreviews.push({
        row: parsed.row,
        sheet: parsed.sheet,
        ...
        pricingRule: null,
        warning: null,
        error: 'Giá bán tính được (priceBuy) không hợp lệ — phải lớn hơn 0.',
        _isError: true,
        _slug: null,
        _code: parsed.code,
        _description: parsed.description,
