...
describe('lib/apiAuth', () => {
  ...
  describe('authenticate', () => {
    ...
    it('ném AuthError 500 khi query profiles lỗi hạ tầng (không phải do không có row)', async () => {
    ...
    it('ném AuthError 401 khi query profiles thành công nhưng không có row nào (không có error)', async () => {
      getUserMock.mockResolvedValue({ data: { user: { id: 'u1', email: 'a@simdulich.vn' } }, error: null });
      fromMock.mockReturnValue(createQueryBuilderMock({
        data: null,
        error: null,
