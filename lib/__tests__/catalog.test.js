...
describe('lib/catalog', () => {
  ...
  describe('searchProducts', () => {
    ...
    it('page/size là NaN (vd. Number("abc")) phải fallback về page=1, size=12 thay vì lan truyền NaN', async () => {
      const query = createQueryBuilderMock({ data: [], count: 0, error: null });
      fromMock.mockReturnValue(query);

      const result = await searchProducts({ page: NaN, size: NaN });
    ...
    it('maxPrice là NaN phải fallback về mặc định 1000000 thay vì truyền NaN vào .lte()', async () => {
      const query = createQueryBuilderMock({ data: [], count: 0, error: null });
      fromMock.mockReturnValue(query);

      await searchProducts({ maxPrice: NaN });
    ...
    it('search chứa dấu *: phải quote để PostgREST không hiểu * là alias của %', async () => {
      const categoriesQuery = createQueryBuilderMock({ data: [], error: null });
      const productsQuery = createQueryBuilderMock({ data: [], count: 0, error: null });
      fromMock.mockImplementation((table) => (table === 'categories' ? categoriesQuery : productsQuery));

      await searchProducts({ search: 'Sim 4G*' });

      // PostgREST coi `*` là alias của `%` trong giá trị like/ilike; nếu không
      // quote, `*` sẽ bị hiểu thành wildcard thay vì ký tự literal người dùng
      // gõ vào, khiến kết quả khớp rộng hơn một cách âm thầm.
      expect(productsQuery.or).toHaveBeenCalledWith('title.ilike."%Sim 4G*%"');
    });

    it('search chứa cả dấu phẩy và dấu *: quote phải vô hiệu hoá toàn bộ ký tự đặc biệt, không chỉ một loại', async () => {
      const categoriesQuery = createQueryBuilderMock({ data: [], error: null });
      const productsQuery = createQueryBuilderMock({ data: [], count: 0, error: null });
      fromMock.mockImplementation((table) => (table === 'categories' ? categoriesQuery : productsQuery));
