...
describe('processSepayWebhook', () => {
  ...
  it('webhook gọi lại (transaction_ref trùng) nhưng đơn vẫn pending -> vẫn cập nhật paid', async () => {
    const selectOrderQuery = createQueryBuilderMock({ data: { id: 1, total_amount: 178000, payment_status: 'pending' }, error: null });
    const updateOrderQuery = createQueryBuilderMock({ data: null, error: null });
    const insertTxQuery = createQueryBuilderMock({ data: null, error: { code: '23505' } });
    let orderCallCount = 0;
    fromMock.mockImplementation((table) => {
      if (table === 'orders') { orderCallCount += 1; return orderCallCount === 1 ? selectOrderQuery : updateOrderQuery; }
      if (table === 'payment_transactions') return insertTxQuery;
      throw new Error(`unexpected table ${table}`);
    });
    ...
    expect(updateOrderQuery.update).toHaveBeenCalledWith({ payment_status: 'paid' });
    expect(updateOrderQuery.eq).toHaveBeenCalledWith('id', 1);
    expect(updateOrderQuery.eq).toHaveBeenCalledWith('payment_status', 'pending');
  });

  it('webhook gọi lại (transaction_ref trùng) và đơn đã paid -> không ghi đè order', async () => {
    const selectOrderQuery = createQueryBuilderMock({ data: { id: 1, total_amount: 178000, payment_status: 'paid' }, error: null });
    const insertTxQuery = createQueryBuilderMock({ data: null, error: { code: '23505' } });
    fromMock.mockImplementation((table) => (table === 'orders' ? selectOrderQuery : insertTxQuery));

    const result = await processSepayWebhook(serviceClientMock, { transferType: 'in', content: 'CK toi don SDL2345ABCD', transferAmount: 178000, id: 42 });

    expect(result).toEqual({ matched: true, duplicate: true });
    expect(selectOrderQuery.update).not.toHaveBeenCalled();
  });

  it('lỗi truy vấn đơn hàng -> ghi api_logs riêng và trả order_lookup_failed', async () => {
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const orderQuery = createQueryBuilderMock({ data: null, error: { message: 'connection timeout' } });
    const logQuery = createQueryBuilderMock({ data: null, error: null });
    fromMock.mockImplementation((table) => {
      if (table === 'orders') return orderQuery;
      if (table === 'api_logs') return logQuery;
      throw new Error(`unexpected table ${table}`);
    });

    const result = await processSepayWebhook(serviceClientMock, { transferType: 'in', content: 'CK toi don SDL2345ABCD', transferAmount: 178000, id: 42 });

    expect(result).toEqual({ matched: false, reason: 'order_lookup_failed' });
    expect(logQuery.insert).toHaveBeenCalledWith(expect.objectContaining({ api_type: 'SEPAY_WEBHOOK_ORDER_LOOKUP_ERROR' }));
...
describe('simulatePaymentWebhook', () => {
  ...
  it('lỗi truy vấn đơn hàng -> trả order_lookup_failed', async () => {
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    fromMock.mockReturnValue(createQueryBuilderMock({ data: null, error: { message: 'connection timeout' } }));
  ...
  it('insert trùng transaction_ref (23505) nhưng đơn vẫn pending -> vẫn cập nhật paid', async () => {
    const selectOrderQuery = createQueryBuilderMock({ data: { id: 1, payment_status: 'pending' }, error: null });
    const updateOrderQuery = createQueryBuilderMock({ data: null, error: null });
    const insertTxQuery = createQueryBuilderMock({ data: null, error: { code: '23505' } });
    let orderCallCount = 0;
    fromMock.mockImplementation((table) => {
  ...
  it('insert trùng transaction_ref (23505) và đơn đã paid -> không ghi đè order', async () => {
    const selectOrderQuery = createQueryBuilderMock({ data: { id: 1, payment_status: 'paid' }, error: null });
    const insertTxQuery = createQueryBuilderMock({ data: null, error: { code: '23505' } });
    fromMock.mockImplementation((table) => (table === 'orders' ? selectOrderQuery : insertTxQuery));
