...
describe('runSmartImport — Critical/Important fixes', () => {
  ...
  it('summary trả về skipped=1 cho slug trùng, và created+updated+failed+skipped === totalRows', async () => {
    const categoriesSelectQuery = createQueryBuilderMock({ data: [{ id: 1, name: 'Japan' }], error: null });
    const productsSelectQuery = createQueryBuilderMock({ data: [], error: null });
    const productsUpsertQuery = createQueryBuilderMock({ data: [{ slug: 'japan-1gb-1-day-wm-jp-1d' }], error: null });
    ...
    const skippedRowsCount = summary.rows.filter((r) => r.status === 'skipped').length;
    expect(skippedRowsCount).toBe(1);
    expect(summary.skipped).toBe(skippedRowsCount);
    expect(summary.created + summary.updated + summary.failed + summary.skipped).toBe(summary.totalRows);
  });
