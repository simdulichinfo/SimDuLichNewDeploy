...
describe('computePriceBuyFromRow', () => {
  ...
  it('markupPercent mode dùng mặc định 30% khi markupPercent null/thiếu', () => {
    expect(computePriceBuyFromRow({ priceImport: 100000 }, 'markupPercent', { markupPercent: null })).toBe(130000);
    expect(computePriceBuyFromRow({ priceImport: 100000 }, 'markupPercent', {})).toBe(130000);
  });

  it('daysPlusFee mode dùng mặc định 14000 khi fixedFee null/thiếu', () => {
    expect(computePriceBuyFromRow({ priceImport: 100000, durationDays: 5 }, 'daysPlusFee', { fixedFee: null })).toBe(514000);
    expect(computePriceBuyFromRow({ priceImport: 100000, durationDays: 5 }, 'daysPlusFee', {})).toBe(514000);
  });
});
...
describe('runProductImport', () => {
  ...
  it('đánh dấu failed khi thiếu durationDays hoặc giá trị không phải số', async () => {
    fromMock.mockReturnValue(createQueryBuilderMock({ data: { id: 5 }, error: null }));

    const buffer = (function build() {
      const wb = XLSX.utils.book_new();
      const sheet = XLSX.utils.json_to_sheet([
        { categorySlug: 'nhat-ban', title: 'A', slug: 'a', simType: 'esim', priceImport: 50000, dataInfo: '1GB', status: 'active', priceBuy: 65000 },
      ]);
      XLSX.utils.book_append_sheet(wb, sheet, 'Sheet1');
      return XLSX.write(wb, { type: 'buffer', bookType: 'csv' });
    })();
    ...
    expect(summary.failed).toBe(1);
    expect(summary.rows[0].status).toBe('failed');
    expect(summary.rows[0].message).toBe('Thiếu số ngày sử dụng (durationDays).');
  });

  it('đánh dấu failed khi durationDays là ô trống (blank cell -> null qua defval:null), không được ghi DB', async () => {
    const categoryQuery = createQueryBuilderMock({ data: { id: 5 }, error: null });
    const productsFromMock = vi.fn(() => {
      throw new Error('không được gọi DB cho bảng products khi durationDays trống');
    });
    fromMock.mockImplementation((table) => {
      if (table === 'categories') return categoryQuery;
      return productsFromMock(table);
    });

    // aoa_to_sheet with an `undefined` cell reproduces sheet_to_json's defval:null
    // behavior exactly: the column exists in the header but the row's cell is absent,
    // so parseImportRows yields durationDays: null (not 0, not NaN).
    const aoa = [
      ['categorySlug', 'title', 'slug', 'simType', 'priceImport', 'dataInfo', 'durationDays', 'status', 'priceBuy'],
      ['nhat-ban', 'A', 'a', 'esim', 50000, '1GB', undefined, 'active', 65000],
    ];
    ...
    expect(summary.rows[0].status).toBe('failed');
    expect(summary.rows[0].message).toBe('Thiếu số ngày sử dụng (durationDays).');
    expect(productsFromMock).not.toHaveBeenCalled();
  });

  it('đánh dấu failed khi durationDays = 0 (không phải giá trị ngày hợp lệ)', async () => {
    fromMock.mockReturnValue(createQueryBuilderMock({ data: { id: 5 }, error: null }));

    const buffer = buildCsvBuffer([
      { categorySlug: 'nhat-ban', title: 'A', slug: 'a', simType: 'esim', priceImport: 50000, dataInfo: '1GB', durationDays: 0, status: 'active', priceBuy: 65000 },
    ]);
  ...
  it('đánh dấu failed khi priceImport là ô trống (blank cell -> null qua defval:null), không được ghi DB', async () => {
    const categoryQuery = createQueryBuilderMock({ data: { id: 5 }, error: null });
    const productsFromMock = vi.fn(() => {
      throw new Error('không được gọi DB cho bảng products khi priceImport trống');
    });
    fromMock.mockImplementation((table) => {
      if (table === 'categories') return categoryQuery;
      return productsFromMock(table);
    ...
    const aoa = [
      ['categorySlug', 'title', 'slug', 'simType', 'priceImport', 'dataInfo', 'durationDays', 'status', 'priceBuy'],
      ['nhat-ban', 'A', 'a', 'esim', undefined, '1GB', 3, 'active', 65000],
    ];
    const sheet = XLSX.utils.aoa_to_sheet(aoa);
    const rows = XLSX.utils.sheet_to_json(sheet, { defval: null });
    expect(rows[0].priceImport).toBeNull();
    ...
    expect(summary.failed).toBe(1);
    expect(summary.rows[0].status).toBe('failed');
    expect(summary.rows[0].message).toBe('Thiếu giá nhập (priceImport).');
    expect(productsFromMock).not.toHaveBeenCalled();
  });

  it('đánh dấu failed khi priceImport = 0', async () => {
    fromMock.mockReturnValue(createQueryBuilderMock({ data: { id: 5 }, error: null }));

    const buffer = buildCsvBuffer([
      { categorySlug: 'nhat-ban', title: 'A', slug: 'a', simType: 'esim', priceImport: 0, dataInfo: '1GB', durationDays: 3, status: 'active', priceBuy: 65000 },
    ]);
  ...
  it('đánh dấu failed khi priceBuy = 0 trong file (manual mode) — không được ghi DB', async () => {
    const categoryQuery = createQueryBuilderMock({ data: { id: 5 }, error: null });
    const productsFromMock = vi.fn(() => {
      throw new Error('không được gọi DB cho bảng products khi priceBuy = 0');
    });
    fromMock.mockImplementation((table) => {
      if (table === 'categories') return categoryQuery;
      return productsFromMock(table);
    ...
    const buffer = buildCsvBuffer([
      { categorySlug: 'nhat-ban', title: 'A', slug: 'a', simType: 'esim', priceImport: 50000, dataInfo: '1GB', durationDays: 3, status: 'active', priceBuy: 0 },
    ]);
  ...
  it('đánh dấu failed khi priceBuy âm trong file (manual mode) — không được ghi DB', async () => {
    const categoryQuery = createQueryBuilderMock({ data: { id: 5 }, error: null });
    const productsFromMock = vi.fn(() => {
      throw new Error('không được gọi DB cho bảng products khi priceBuy âm');
    });
    fromMock.mockImplementation((table) => {
      if (table === 'categories') return categoryQuery;
      return productsFromMock(table);
    ...
    const buffer = buildCsvBuffer([
      { categorySlug: 'nhat-ban', title: 'A', slug: 'a', simType: 'esim', priceImport: 50000, dataInfo: '1GB', durationDays: 3, status: 'active', priceBuy: -5000 },
    ]);
